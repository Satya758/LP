...
class LPFormatGrammar : public qi::grammar<Iterator, ParsedObject(), Skip> {
 public:
  ...
  qi::rule<Iterator, SCN(), Skip> objective;
  ...
  qi::rule<Iterator, SCN(), Skip> constraintsLHS;
...
Problem LPFormatParser::getProblem(ParsedObject& parsedObj) const {
  ...
  int index = 0;
  for (const SCN& scs : parsedObj.objective) {
...
// Convert to problem
Problem LPFormatParser::getProblem(const std::vector<CT>& constraintTriplets,
 ...
 */
void LPFormatParser::removeRedundantColumns(Problem& problem) const {
...
std::vector<CT> LPFormatParser::getConstraintTriplets(
...
int LPFormatParser::getColIndex(const std::string& name,
                                std::unordered_map<std::string, int>& nameMap,
 ...
 */
void LPFormatParser::addBounds(const ParsedObject& parsedObj,
                               std::unordered_map<std::string, int>& nameMap,
                               std::vector<CT>& constraintTriplets,
                               std::vector<double>& c,
                               std::vector<double>& h) const {
 ...
 * pieces to bounds
 */
void LPFormatParser::addMissingBounds(
 ...
 */
std::vector<std::string> LPFormatParser::getRowSingletonConstraints(
...
void LPFormatParser::printInCVXSparseForm(const Problem& problem1) const {
